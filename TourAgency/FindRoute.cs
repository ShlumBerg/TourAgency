...
namespace TourAgency
{
    public partial class FindRoute : Form
    {
        ...
        List<string> CityRes = new List<string>();
        public FindRoute()
        {
            InitializeComponent();
            ...
            dataAdapter = new MySqlDataAdapter($"select * from departure_cities", Form1.connection);
            dataTable = new DataTable();
            dataAdapter.Fill(dataTable);
            foreach (DataRow row in dataTable.Rows)
            {
                CityRes.Add((string)row[1]);
                CityIds.Add((int)row[0]);
            }
            ...
            comboBoxCity.DataSource = CityRes;
        }
        ...
        {
            List<bool> Enabled = new List<bool> { checkBoxRoute.Checked,checkBoxCity.Checked,checkBoxDescription.Checked,enableHotelId, enableRegion, enableFoods, enableCountry, enableAge, enablePool, enableStars, enableName, !radioButton3.Checked, !radioButton5.Checked };
            int regionId = -1;
            if (enableRegion)
            {
            ...
            if (checkBoxCity.Checked) 
            {
                ...
                {
                    cityId = CityIds[comboBoxCity.SelectedIndex];
                }
            }

            FindRouteViewer form = new FindRouteViewer(Enabled, Convert.ToInt32(numericUpDownRoute.Value),cityId,textBoxDescription.Text,Convert.ToInt32(HotelIDnumericUpDown2.Value), regionId, foodsId, countryId, ageId, poolId, Convert.ToInt32(numericUpDown1.Value), textBoxName.Text, radioButton1.Checked, radioButton4.Checked);
            form.ShowDialog();
        }
        ...
        private void FindRoute_Load(object sender, EventArgs e)
        {
        ...
        {
            numericUpDownRoute.Enabled = ((CheckBox)sender).Checked;
        }

        private void checkBoxCity_CheckedChanged(object sender, EventArgs e)
        {
            comboBoxCity.Enabled = ((CheckBox)sender).Checked;
        }
